...
class Partner_controller extends CI_Controller {
   ...
   public $queryString = 'SELECT id, name, type, logo, icon, url, display FROM Partner';
  ...
  {
    $queryResult = $this->partner_model->getQueryData($this->queryString, TRUE);
    $data['queryResult'] = $queryResult;

    //load view
    $data['main_content'] = 'partner_view';
  ...
  {
    $queryResult = $this->partner_model->getQueryData($this->queryString, FALSE);
    $returnArray = array();
  ...
  function create() 
  {
    ...
    $queryResult = $this->partner_model->create($name, $type, $logo, $icon, $url, $display);
    if ($queryResult) {
      $returnCode = "S";
  ...
  function delete() 
  {
    ...
    $queryResult = $this->partner_model->delete($id);
    if ($queryResult) {
      $returnCode = "S";
  ...
  function update() 
  {
    ...
    $queryResult = $this->partner_model->update($id, $name, $type, $logo, $icon, $url, $display);
    if ($queryResult) {
      $returnCode = "S";
