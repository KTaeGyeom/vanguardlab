...
class Party_controller extends CI_Controller {
   ...
   public $queryString = 'SELECT id, name, type, logo, icon, url, display FROM Party ORDER BY name';
  ...
  {
    $queryResult = $this->party_model->getQueryData($this->queryString, TRUE);
    $data['queryResult'] = $queryResult;

    //load view
    $data['main_content'] = 'party_view';
  ...
  {
    $queryResult = $this->party_model->getQueryData($this->queryString, FALSE);
    $returnArray = array();
  ...
  function create() 
  {
    ...
    $queryResult = $this->party_model->create($name, $type, $logo, $icon, $url, $display);
    if ($queryResult) {
      $returnCode = "S";
  ...
  function delete() 
  {
    ...
    $queryResult = $this->party_model->delete($id);
    if ($queryResult) {
      $returnCode = "S";
  ...
  function update() 
  {
    ...
    $queryResult = $this->party_model->update($id, $name, $type, $logo, $icon, $url, $display);
    if ($queryResult) {
      $returnCode = "S";
